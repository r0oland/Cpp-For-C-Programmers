...
void dijkstra(GraphMatrix G, int src) {
  ...
  for (int count = 0; count < nNodes - 1; count++) {
    ...
    int u = minDistance(dist, sptSet, nNodes);
...
int main() {
  ...
  // TODO ----------------------------------------------
  // Compute for a set of randomly generated graphs an average shortest
  // path.
